...
//            die;
        ...
        $data['footerJs'][4] = base_url('uploads/custom/project/project.js');
...
//echo $completed_count;exit;
		...
		//$data['completed'] = $this->mongo_db->where(array('is_deleted'=>0, 'project_status'=>3))->get('project_master');
		
		
		/* dishit code for completed project starts*/
		$rt = array();
		if (count($data['dataset']) > 0) {
            foreach ($data['dataset'] as $proj) {
