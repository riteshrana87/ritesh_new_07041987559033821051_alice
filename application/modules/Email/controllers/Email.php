...
class Email extends Public_controller {
    ...
    function index() {
		...
		$data['pageTitle'] = lang('your_emails');
		$data['headerCss'][0] = base_url('uploads/custom/Developer.css');
		$data['footerJs'][0] = base_url('uploads/custom/Email/email.js');
		...
		$client_data =  $this->Email_model->getClientData($user_id);
		/* print_r($client_data);
		die(); */
        /* $username = "testalice786@gmail.com"; //cmetric2016@outlook.com";
        $password = "alicetest786";
        $encryption = 'SSL'; // or ssl or ''  */
		 
		$username = $client_data[0]['imapusername']; //cmetric2016@outlook.com";
        $password = $client_data[0]['imappassword'];
        $encryption = 'SSL'; // or ssl or '' 
		
        //if ($imap->connect('imap-mail.outlook.com:993/', $username, $password, $encryption)) {
		if($this->load->is_loaded('imap'))
		{
       ...
       /*  $imap->getQuotaUsage();
		...
		//$saveactivity = $this->save_activity($this->userId,'Activity called from Email Class.','Email');
    }
...
// @file_put_contents($abspath . $uniqueFilename, $attachmentArray[$key]['attachment']);
    ...
    public function getEmailConfigData($userId = '', $fields = '') {
        ...
        $configData =  $this->mongo_db->get_where('EmailSettings', array('user_id' => new \MongoId($userId)));
