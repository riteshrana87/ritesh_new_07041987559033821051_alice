...
class Cron extends CI_Controller {
    ...
    public function recurring() {
        ...
        echo $now   = date('m/d/Y')."</br>"; // current date

        $recurring=$this->mongo_db->get('invoice_recurring');
       ...
       if(!empty($recurring))
       {
            ...
            if(!empty($row['next_issue_date'])){
				
                ///$this->notify_user($row['created_by'], $row);
              
                $current_date    = date('Y-m-d');
                
                $remind_date = $row['next_issue_date'] ;
                ...
                if($howoften == '1'){
						...
						$rec_date=date('m/d/Y', $date);
						if($row['next_issue_date'] == $rec_date){
							
							//die;
							pr($row);
							echo "weekly data match";
				...
				if($howoften == '2'){
						...
						$date_rec = date('m/d/Y', strtotime('+1 month'));
						//$rec_date=date('m/d/Y', $date);
						if($row['next_issue_date'] == $date_rec){
							pr($row);
							echo "weekly data match";
							//die;
				...
				if($howoften == '3'){
						...
						$date_rec = date('m/d/Y', strtotime('+3 month'));
						//$rec_date=date('m/d/Y', $date);
						if($row['next_issue_date'] == $date_rec){
							
							pr($row);
							echo "quaterly data match ";
				...
				if($howoften == '4'){
						...
						$date_rec = date('m/d/Y', strtotime('+12 month'));
						//$rec_date=date('m/d/Y', $date);
						if($row['next_issue_date'] == $date_rec){
							
							pr($row);
							echo "yearly data match";
    ...
    public function reminders() {
        ...
        $reminder_data=$this->mongo_db->get('send_reminder');
        
       if(!empty($reminder_data))
       {
        
            foreach ($reminder_data as $row) {
            $invoice_data=$this->mongo_db->get_where('invoices',array('invoice_code' =>$row['invoice_id']));
            //pr($invoice_data[0]);
           // die();
            $created_date=strtotime($invoice_data[0]['created_date']);
            ...
            if($row['reminder_type']==1){
						...
						$after_date = strtotime("+".$row['days']." day",$date);
            ...
            if($row['reminder_type']==3){
					...
					$reminder_date=date('m/d/Y', $row['days']);
