...
class MongoPagination {
    ...
    public function Paginate() {
        ...
        $whereNot = (!empty($this->query['#wherenot'])) ? $this->query['#wherenot'] : array();
        $where_lt = (!empty($this->query['#where_lt'])) ? $this->query['#where_lt'] : array();
        //  $whereIn = (!empty($this->query['#wherenot'])) ? $this->query['#wherenot'] : array();
        //  Get total results count
        $this->totalItemCount = $this->mongo_db->where($find)->count($collection);
        
       // echo $where_lt[key($where_lt)];die;
        /* 	Enable Limit based Query	 */
        if (!empty($this->limitResult)) {

            if (!empty($whereNot) && empty($where_lt)) {
                $resultSet = $this->mongo_db
                        ->select($select)
                        ->where($find)
                        ->where_not_in(key($whereNot), $whereNot[key($whereNot)])
                        ->order_by($sort)
                        ->limit($this->limitResult)
                        ->get($collection);
            } else if (empty($whereNot) && !empty($where_lt)) {
                $resultSet = $this->mongo_db
                        ->select($select)
                        ->where($find)
                        ->where_lt(key($where_lt),$where_lt[key($where_lt)])
                        ->order_by($sort)
                        ->limit($this->limitResult)
                        ->get($collection);
            } else if (!empty($whereNot) && !empty($where_lt)) {
        ...
        /* 	Enable Pagination based Query	 */ else {
    ...
    public function getPageLinks($setVisiblePagelinkCount = 3, $type = 'HTML') {
        ...
        //$html .= '<li class="disabled"><a href="'.$this->preparePageLink($this->baseOffset).'"><i class="fa fa-angle-left"></i></a></li>';
        if (0 != $this->currentOffset) {
            $html .= '<li ><a href="' . $this->preparePageLink($this->currentOffset - $this->itemsPerPage) . '"><i class="fa fa-angle-left"></i></a></li>';
        }
        ...
        for ($i = $current_page; $i <= $end; $i++) { // Shows page numbers from current page
            ...
            if ($VisiblePagelinkCount <= $setVisiblePagelinkCount) {
                if ($current_page == $i) {
                    $html .= '<li class="active"><a  href="' . $this->preparePageLink($this->currentOffset) . '">' . ($i) . '</a></li>';
                } else {
                    $html .= '<li><a href="' . $this->preparePageLink(( $i * $this->itemsPerPage) - ($this->itemsPerPage)) . '">' . ($i) . '</a></li>';
                }
            }
            $VisiblePagelinkCount++;
        }
        if ($this->totalPages > $current_page) {
            $html .= '<li><a href="' . $this->preparePageLink($this->currentOffset + $this->itemsPerPage) . '"><i class="fa fa-angle-right"></i> </a></li>';
        }
        //$html .= '<li><a href="'.$this->preparePageLink(($this->totalPages -1) * $this->itemsPerPage).'">Last</a></li>';
        $html .= '</ul>';
