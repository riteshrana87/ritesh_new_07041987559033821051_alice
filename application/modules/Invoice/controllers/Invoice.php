...
//        }
      ...
      public function Invoice_to($view = 'grid') {
		

        $data['footerJs'][0] = base_url('uploads/assets/plugins/parsleyjs/dist/parsley.min.js');
        $data['footerJs'][1] = base_url('uploads/assets/js/filterizr/jquery.filterizr.js');
        $data['footerJs'][9] = base_url('uploads/custom/invoice/invoice.js');
        $data['footerJs'][2] = base_url('uploads/assets/plugins/bootstrap-datepicker/dist/js/bootstrap-datepicker.min.js');
        $data['footerJs'][3] = base_url('uploads/assets/plugins/bootstrap-daterangepicker/daterangepicker.js');
        ...
        $data['footerJs'][7] = base_url('uploads/assets/plugins/datatables/jquery.dataTables.min.js');
        $data['footerJs'][8] = base_url('uploads/assets/plugins/datatables/dataTables.responsive.min.js');
...
//        }
    ...
    function invoice_auto_gen_Id() {
     ...
     function invoice_to_auto_gen_Id() {
        return 'PINV' . mt_rand(100000, 999999);
    }
    ...
    public function Add($client_id='') {
		...
		if(!empty($client_id)){
			...
			$data['selected_client_details'] = $this->mongo_db->get_where('Client',array('_id' => new \MongoId($client_id)));
		}
	...
	//$id=$this->session->userdata('alice_session')['_id'];
      ...
      public function Add_to($client_id='') {
        $data['main_content'] = '/AddInvoice';
	...
	//$id=$this->session->userdata('alice_session')['_id'];
        ...
        $data['invoice_auto_id'] = $this->invoice_to_auto_gen_Id();
    ...
    public function Edit($invoice_id) {
        ...
        if ($invoice_id != '') {
			...
			//print_r($data['editrecord'][0]['invoice_code']);
			$invoice_code = $data['editrecord'][0]['invoice_code'];
			
			$data['send_reminder']=$this->mongo_db->get_where('send_reminder', array('invoice_id' => $invoice_code));
...
//        $fileName = implode(',', $images);
    ...
    public function insertdata() {
        ...
        $invoice_data['currency'] = $currency[0]['company_currency'];//$this->input->post('country_info');
        ...
        $invoice_data['save_type'] = $this->input->post('HdnSubmitBtnVlaue');
         $invoice_data['invoice_type'] = $this->input->post('invoice_type');
    ...
    public function DownloadPDF($id, $section = NULL) {
        ...
        */
        ...
        if (!empty($data['editRecord'][0]['currency']) && isset($data['editRecord'][0]['currency']) && $data['editRecord'][0]['currency'] != "") {
            ...
            $PDFCuntArray = $data['editRecord'][0]['currency'];
        }
...
//            echo $subtotal;exit;
	...
	public function curlanAdd() {
        ...
        $client_idcurlang = $this->input->post('client_idcurlang');
			...
			$data_invoice = $this->mongo_db->get_where('invoices',array('invoice_code' => $invoice_idcurlang));
			
			if(count($data_invoice)>0){
				$this->mongo_db->where('invoice_code', $invoice_idcurlang)->set($data)->update('invoices');
				$saveactivity = $this->save_activity($this->session->userdata('alice_session')['_id'],lang('invoice_currency_update'),'Invoice');
				$error_msg = SUCCESS_START_DIV_NEW . lang('invoice_currency_update') . SUCCESS_END_DIV;
				$this->session->set_flashdata('message', $error_msg);
				$result = array('status'=>true,'res'=>$error_msg);
                echo json_encode($result);
			}
			else{
				$error_msg = SUCCESS_START_DIV_NEW . lang('save_invoice_fist') . SUCCESS_END_DIV;
				$this->session->set_flashdata('message', $error_msg);
				$result = array('status'=>true,'res'=>$error_msg);
                echo json_encode($result);
			}
