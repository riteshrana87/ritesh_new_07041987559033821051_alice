...
if (count($product) > 0) {
    ...
    foreach ($product as $prod) {
        ...
        <div class="form-group pdiv" id="productDiv_<?php echo $p; ?>">
            ...
            <div class="col-md-2">
                <select class="select2 form-control" name="product_category[]" id="product_category<?php echo $p; ?>" placeholder="<?php echo lang('category'); ?>">
            ...
            <div class="col-md-2">
                        <select class="select2 form-control pro_tax" name="product_tax[]" id="product_tax<?php echo $p; ?>" placeholder="<?php echo lang('tax'); ?>">
                        <option value="0" selected=""></option>
            <?php
            if (count($tax) > 0) {
                foreach ($tax as $tx) {
                    ?>
							<option data-taxrate="<?php echo $tx['tax'];?>" data-taxname="<?php echo $tx['tax_name'];?>" <?php echo ($prod['product_tax'] == $tx['_id']) ? 'selected' : ''; ?> value="<?php echo $tx['_id']; ?>"><?php echo $tx['tax_name'].' ('.$tx['tax'].'%)' ; ?></option>
                <?php } ?>
            <?php } ?>
            ...
            <div class="col-md-2">
                <input type="text" name="product_price[]" min="0" maxlength="6" id="product_price<?php echo $p; ?>" onkeypress="return numericDecimal(event)" class="form-control product_price" data-inc="<?php echo $p; ?>" value="<?php echo $prod['product_price']; ?>" placeholder="<?php echo lang('price'); ?> *" data-parsley-pattern="/^\d{0,8}(\.\d{0,2})?$/" required >
            </div>
            <?php if ($p > 0) { ?>
                <div class="col-md-1">
