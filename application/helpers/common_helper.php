...
function getMailConfig() {
    ...
    $CI = & get_instance();
    $id = $CI->session->userdata['alice_session']['_id'];
    $defaultDashboard = $CI->mongo_db->get_where('EmailSettings', array('user_id' => new \MongoId($id)));
    $configData = $defaultDashboard;
    return $configData;
}

function getMailConfigForgotpassword($to) {
	
    $CI = & get_instance();
    $id = $CI->mongo_db->get_where('User', array('email' => $to));
   
    $defaultDashboard = $CI->mongo_db->get_where('EmailSettings', array('user_id' => new \MongoId($id[0]['_id'])));
    /*pr($defaultDashboard);
    die();*/
    $configData = $defaultDashboard;
    return $configData;
...
//	pr($attach['pdf_file']);
//        pr($attach['file_attach']);
//        exit;
    $CI = & get_instance();
    if(isset($CI->session->userdata['alice_session']['_id'])){
		$configs = getMailConfig();
		}
		else{
			$configs = getMailConfigForgotpassword($to);
			}
                        
     // Get Email configs from Email settigs page
    //$CI->load->library('parser');
    if (!empty($configs)) {
		//die('here');
        $config['protocol'] = 'smtp';
        $config['smtp_host'] = $configs[0]['smtphost']; //change this
        $config['smtp_port'] = $configs[0]['smtpport'];
        $config['smtp_user'] = $configs[0]['smtpfromemail']; //change this
        $config['smtp_pass'] = $configs[0]['smtpupassword']; //change this
        $config['mailtype'] = 'html';
        $config['charset'] = 'iso-8859-1';
        $config['wordwrap'] = TRUE;
        $config['newline'] = "\r\n"; //use double quotes to comply with RFC 8
        $CI->load->library('email', $config); // load email library
        $CI->email->from($configs[0]['email'], "CMS TEST");
        $CI->email->to($to);
        $CI->email->subject($subject);
        $CI->email->message($message);
        if (isset($attach) && $attach != "") {
            if(!empty($attach['pdf_file'])){
                $CI->email->attach($attach['pdf_file']); // attach file /path/to/file1.png
            }elseif(!empty($attach['file_attach'])){
                $CI->email->attach($attach['file_attach']); // attach file /path/to/file1.png
            }
        }
...
function systemTemplateDataBySlug($st_slug) {
    ...
    $email_template_data = $CI->mongo_db->get_where('email_templates', array('template_title' => $st_slug));
...
function productName($plroduct_id) {
    $ci = & get_instance();
    $product_data = $ci->mongo_db->get_where('product', array('_id' => new \MongoId($plroduct_id)));
