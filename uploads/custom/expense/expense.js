function calculate_subtotal()
{
    ...
    $('.product_price').each(function () {
		...
		var rt = (parseFloat(qtycalc) * parseFloat(selected_taxrate)) / parseFloat(100) || 0;
		
		if(txnm == selected_taxname){
			txnm = selected_taxname;
		}
		else{
			txnm = selected_taxname;
		}
		...
		total_tax = total_tax + rt;
		
    });
	$('#amount_tax').html(total_tax);
    var taxsum = 0;
    var calculatedtaxval = 0;
    var finalamt = 0;
	var i = 0;
   
   /* $('.pro_tax option:selected').each(function () {

        taxrate = $(this).attr('data-taxrate');
        taxname = $(this).attr('data-taxname');
		
		obj[taxname+"("+taxrate+"%)"] = i;
    ...
    $('#total_text').html(total_tax + subtotal);
	
    $('#total').val(total_tax + subtotal);
    //total_text,toal
