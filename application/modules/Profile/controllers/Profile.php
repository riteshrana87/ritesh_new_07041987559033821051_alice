       ...
       /*  $this->form_validation->set_rules('profile_picture', 'profile_picture', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
		   ...
		   $files = $_FILES;
			...
			// Profile Picture
				...
				$profile_fileName = str_replace(' ', '_', $profile_fileName);
		...
		//echo $this->session->userdata['alice_session']['firstname'].' '.$this->session->userdata['alice_session']['lastname'];
           ...
           //$this->mongo_db->insert('User', $data);
			...
			$saveactivity = $this->save_activity($this->session->userdata('alice_session')['_id'],lang('profile_add_msg'),'Profile');
...
//        $fileName = implode(',', $images);
	...
	public function Insert_statutoryData() {
        ...
        $this->form_validation->set_error_delimiters(ERROR_START_DIV_NEW, ERROR_END_DIV);
        $this->form_validation->set_rules('tin_number', 'tin_number', 'trim|required');
        $this->form_validation->set_rules('cst_number', 'cst_number', 'trim|required');
        $this->form_validation->set_rules('pan_number', 'pan_number', 'trim|required');
        $this->form_validation->set_rules('tan_number', 'tan_number', 'trim|required');
        $this->form_validation->set_rules('cin', 'cin', 'trim|required');
        $this->form_validation->set_rules('servicetax_number', 'servicetax_number', 'trim|required');
